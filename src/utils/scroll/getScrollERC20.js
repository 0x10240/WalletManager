...
async function getScrollERC20(address) {
    try {
        ...
        let usdtUrl = `https://blockscout.scroll.io/api?module=account&action=tokenbalance&contractaddress=0xf55bec9cafdbe8730f096aa55dad6d22d44099df&address=${address}`;
        const response = await axios.get(usdtUrl);
        const balance = parseFloat(response.data?.result / Math.pow(10, decimals)).toFixed(2);
        let usdcUrl = `https://blockscout.scroll.io/api?module=account&action=tokenbalance&contractaddress=0x06eFdBFf2a14a7c8E15944D1F4A48F9F95F663A4&address=${address}`;
